...
const VaultBox = ({poppup, setPoppup, tokenBalance}) => {
    ...
    useEffect(async () => {
        if(account) {
            ...
            const claimableAmount = await monkeyContract.getUserUnclaimedTokens_C(account)
            BigNumber.config({
                EXPONENTIAL_AT: 1000,
                DECIMAL_PLACES: 80,
            })
            const busdAmount = await monkeyContract.tokenToBUSD(new BigNumber(claimableAmount / 2).toString())
            setClaimableAmount(new BigNumber((claimableAmount / 2 / 10 ** 18).toString()).toNumber())
            setBusdAmount(new BigNumber((busdAmount / 10 ** 18).toString()).toNumber())
        }
        if(monkeyContractForRead) {
            ...
            const total = await monkeyContractForRead.methods.totalTokenInCamp().call()
    ...
    return (
        <MonBox>
            <h2>$MONKEY CAMP&nbsp;
                <Tooltip title="The monkey camp is a place where all the monkeys come together . 25% of the daily earnings in mint monkey and stake monkey goes to the monkey camp when you press harvest or compound . This is a safetyfeature to keep the project sustainable.">
            ...
            <br></br>
            <Values>
                <Value title='MONKEY minted' content={claimableAmount != undefined ? claimableAmount.toLocaleString() : '_'}/>
                <Value title='BUSD minted' content={busdAmount != undefined ? busdAmount.toLocaleString() : '_'}/>
            </Values>
            <br></br>
            <ClaimWrapper>
                <Border></Border>
                <StyledButton color={'rgb(80,43,133)'} width={'100%'} disabled={!account || claimableAmount == 0} onClick={handleCompound}>Compound&nbsp;
