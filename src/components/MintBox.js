...
const MintBox = ({poppup, setPoppup, tokenBalance}) => {
    ...
    const handleInput = (event) => {
        setAmount(event.target.value > busdBalance ? busdBalance : event.target.value )
    }

    const { account, library, chainId } = useActiveWeb3React()
    const monkeyContract = useMonkeyContract()
    const monkeyContractForRead = useMonkeyContractForRead()
    const busdContract = useTokenContract('0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56')
    useEffect(async () => {
        if(account) {
            const userAllowance = await busdContract.allowance(account, MONKEY_ADDRESS)
            setAllowance(new BigNumber((userAllowance / 10 ** 18).toString()).toNumber())
            const userBalance = await busdContract.balanceOf(account)
            setBusdBalance(new BigNumber((userBalance / 10 ** 18).toString()).toNumber())
            ...
            const claimableAmount = await monkeyContract.getUserUnclaimedTokens_M(account)
        ...
        if(monkeyContractForRead) {
            ...
            const total = await monkeyContractForRead.methods.totalBUSDStaked().call()
    ...
    const handleStake = useCallback(async () => {
        ...
        monkeyContract.stakeBUSD(referer, new BigNumber(amount).times(new BigNumber(10).pow(18)).toString())
