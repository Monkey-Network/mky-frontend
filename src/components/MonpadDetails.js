...
const MonpadDetails = () => {
  ...
  useEffect(async () => {
    const tokenPrice = await monkeyContractForRead.methods
      .getTokenPrice()
      .call();
    setTokenPrice(new BigNumber((tokenPrice / 10 ** 18).toString()).toNumber());
    const totalSupply = await monkeyContractForRead.methods
      .limitSupply()
      .call();
    setTotalSupply(
      new BigNumber((totalSupply / 10 ** 18).toString()).toNumber()
    );
    ...
    const avaSupply = await monkeyContractForRead.methods
      .availableSupply()
      .call();
  ...
  return (
    <MonPad>
      ...
      <Value
        title="$MONKEY PRICE"
        content={
          tokenPrice.toLocaleString("en-US", { maximumFractionDigits: 6 }) +
          " $"
        }
      />
      <Values>
        ...
        <Value
          ...
          content={circSupply.toLocaleString()}
        />
        <Value title="Available supply" content={avaSupply.toLocaleString()} />
      </Values>
      <StyledLink>
        <a
          href={`https://bscscan.com/address/${MONKEY_ADDRESS}#code`}
          target="_blank"
          data-close="true"
        >
          Contract
        ...
        <a href="#" target="_blank" data-close="true">
          Whitepaper
        </a>
        <font color="#D97D3E">|</font>
        <a href="https://t.me/monkeynetwork" target="_blank" data-close="true">
          Telegram Group
        </a>
        {/* <font color="#D97D3E">|</font>
